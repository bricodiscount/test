...
class AccountTaxReturn(models.TransientModel):
    ...
    def _default_name(self):
        return "Liasse fiscale " + str(date.today().year)
    ...
    year = fields.Selection(LISTE_YEARS, string=_("Année"), default=current_year,
                            help=u"Indique l'année N de la liasse fiscale")
    duration = fields.Integer(string=_("Durée (en mois)"), default=12, help=u"")
        ...
        for l in list_of_radical_account:
            if l in self.list_account_doublon_of_exclude:
                ...
                if l in self.list_account_inverse:
                    ...
                    if l[0] == '_':
                        solde = solde - abs(
                            self.sum_debit(l, date_deb, date_fin)[0] - self.sum_credit(l, date_deb, date_fin)[0])
                    else:
                        solde = solde + self.sum_debit(l, date_deb, date_fin)[0] - \
                                self.sum_credit(l, date_deb, date_fin)[0]
        return solde
    ...
    def get_resultat_net(self, data_list_all):
        ...
        for value in data_list_all:
            if value.get('ref') in ['vente_marchandise', 'achat_marchandises_vendues', 'production_vendue',
                                    ...
                                    'dotations_aux_amortissement', 'dotations_aux_provisions',
                                    'quotes_parts_resultat_operations_commun','produits_financier','charges_financieres',
                                    'produits_exceptionnels', 'charges_exceptionnels'
                                    ]:
                if value.get('signe') == '+':
                    som = som + value.get('solde_n')
                    som_n_1 = som_n_1 + value.get('solde_n_1')
    ...
    def get_solde_intermediaire_gestion_data(self):
        ...
        list_account_value = [
            {'list_G_VMP': ['g', 'Vente  marchandises + Production','t_g0'],
             ...
             'list_achat_marchandises_vendues': ['-/+', "Coût d'achat des marchandises vendues", 'g1', '6037','607','6097'],
             ...
             'list_matieres_p_approvisionnements_c': ['-/+', "Matières premières, approvisionnements consommés", 'g3','601', '602','6031','6032','608', '609','_6097'],
             ...
             'list_autre_achat_charge_externe': ['-', "Autres achats + charges externes", 'g5', '606', '610','611','612','613','614','615','616','617','618','619','621','622','623','624','625','626','627','628','629'],
             ...
             'list_charges_sociales_du_personnel': ['-', "Charges sociales du personnel", 'g6','644', '645','646','647','648','649'],
             'list_G_EBE': ['g', "Excédent brut d'exploitation", 't_g6'],
             'list_autres_produits_gestion_courant': ['+', "Autres produits de gestion courant", 'g7', '75','_755'],
             'list_autres_charge_gestion_courant': ['-', "Autres charges de gestion courant", 'g7', '65','_655'],
             'list_reprises_amortissements_provisions_t_c': ['+', "Reprises amortissements provisions, transferts de charge", 'g7','781', '791'],
             'list_dotations_aux_amortissement': ['-', "Dotations aux amortissements", 'g7', '6811','6812','6868'],
             'list_dotations_aux_provisions': ['-', "Dotations aux provisions", 'g7', '6874','6875','6815','6865','6872','6873'],
             'list_G_REP': ['g', "Résultat d'exploitation", 't_g7'],
             'list_quotes_parts_resultat_operations_commun': ['+', "Quotes parts de résultat sur opérations en commun", 'g8', '755'],
             'list_produits_financier': ['+', "Produits financier", 'g8', '76'],
             'list_charges_financieres': ['-', "Charges financières", 'g8', '66'],
             'list_G_RCO': ['g', "Résultat courant", 't_g8'],
             'list_produits_exceptionnels': ['+', "Produits Exceptionnels", 'g9', '77'],
             'list_charges_exceptionnels': ['-', "Charges Exceptionnels", 'g9', '67'],
    ...
    def get_solde_intermediaire_gestion_detail_data(self):
        ...
        list_account_value = [
            {'list_G_VMP': ['g', 'Vente  marchandises + Production', 't_g0'],
             'list_vente_marchandise': ['+', "vente de marchandises", 'g1', '707', '7097'],
             'list_achat_marchandises_vendues': ['-/+', "Coût d'achat des marchandises vendues", 'g1', '6037',
                                                 '607', '6097'],
             'list_G_MC': ['g', 'Marge Commerçiale', 't_g1'],
             'list_production_vendue': ['+', "Production vendue", 'g2', '701', '702', '703', '704', '705', '706',
                                        '708', '709', '_7097'],
             'list_production_stockee_destockage': ['-/+', "Production stockée ou déstockag", 'g2', '713'],
             'list_production_immobilisee': ['+', "Production immobilisé", 'g2', '72'],
             'list_G_PE': ['g', "Production de l'exercice", 't_g2'],
             'list_matieres_p_approvisionnements_c': ['-/+', "Matières premières, approvisionnements consommés",
                                                      'g3', '601', '602', '6031', '6032', '608', '609', '_6097'],
             'list_sous_traitance_direct': ['-', "Sous traitance direct", 'g3', '604', '605'],
             'list_G_MBP': ['g', "Marge brute de production", 't_g3'],
             'list_G_MBG': ['g', "Marge brute globale", 't_g4'],
             ...
             'list_reprises_amortissements_provisions_t_c': ['+',
                                                             ...
                                                             'g7', '781', '791'],
             'list_dotations_aux_amortissement': ['-', "Dotations aux amortissements", 'g7', '6811', '6812',
                                                  '6868'],
             'list_dotations_aux_provisions': ['-', "Dotations aux provisions", 'g7', '6874', '6875', '6815',
                                               '6865', '6872', '6873'],
             'list_G_REP': ['g', "Résultat d'exploitation", 't_g7'],
             'list_quotes_parts_resultat_operations_commun': ['+',
        ...
        for ref in list_account:
            ...
            for l_account in list_account.get(ref)[3:len(list_account.get(ref))]:
                accounts = self.env["account.account"].search([('code','=like',str(l_account)+'%')])#liste des compte lié a un radical
                for ac in accounts:#solde des comptes inclus dans chaque radical
                    ...
                    if list_account.get(ref)[0] in ['+', '-']:#(solde_n !=0 or solde_n_moins_1 != 0) and
                        sig_data.append(
